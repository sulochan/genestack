...
Genestack is a complete operation and deployment ecosystem for OpenStack services that hevily utilizes cloud native application like
...
of failure compromising the entire system. This concept aligns with the cloud native environments by catagorizing security measures across the lifecycle and
stack of the cloud native technologies.

We aim to follow guidelines from CNCF that models cloud native application into distinct phases that constitute application lifecycle. Security is
injected in each of these phases:
